...
#include "chaps/tpm_utility_nethsm.h"
  ...
  bool Create() {
    ...
    if (result != TSS_SUCCESS) {
      LOG(ERROR) << "Tspi_Context_CreateObject - "
                 << TPMUtilityNetHsm::ResultToString(result);
      return false;
    }
    return true;
  }
...
TPMUtilityNetHsm::TPMUtilityNetHsm(const string& srk_auth_data)
...
TPMUtilityNetHsm::~TPMUtilityNetHsm() {
...
bool TPMUtilityNetHsm::Init() {
  if (is_initialized_)
    return true;
  VLOG(1) << "TPMUtilityNetHsm::Init enter";
  AutoLock lock(lock_);
  ...
  VLOG(1) << "TPMUtilityNetHsm::Init success";
  is_initialized_ = true;
  return true;
}

bool TPMUtilityNetHsm::IsTPMAvailable() {
...
bool TPMUtilityNetHsm::InitSRK() {
  if (!is_initialized_)
    return false;
  if (is_srk_ready_)
    return true;
  VLOG(1) << "TPMUtilityNetHsm::InitSRK enter";
  ...
  VLOG(1) << "TPMUtilityNetHsm::InitSRK success";
  is_srk_ready_ = true;
  return true;
}

bool TPMUtilityNetHsm::Authenticate(int slot_id,
                                  const SecureBlob& auth_data,
                                  const string& auth_key_blob,
                                  const string& encrypted_master_key,
                                  SecureBlob* master_key) {
  VLOG(1) << "TPMUtilityNetHsm::Authenticate enter";
  int key_handle = 0;
  ...
  *master_key = SecureBlob(master_key_str.begin(), master_key_str.end());
  ...
  VLOG(1) << "TPMUtilityNetHsm::Authenticate success";
  return true;
}

bool TPMUtilityNetHsm::ChangeAuthData(int slot_id,
                                    const SecureBlob& old_auth_data,
                                    const SecureBlob& new_auth_data,
                                    const string& old_auth_key_blob,
                                    string* new_auth_key_blob) {
  VLOG(1) << "TPMUtilityNetHsm::ChangeAuthData enter";
  int key_handle = 0;
  ...
  if (!GetKeyBlob(GetTssHandle(key_handle), new_auth_key_blob))
    return false;
  VLOG(1) << "TPMUtilityNetHsm::ChangeAuthData success";
  return true;
}

bool TPMUtilityNetHsm::GenerateRandom(int num_bytes, string* random_data) {
  VLOG(1) << "TPMUtilityNetHsm::GenerateRandom enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  TSS_RESULT result = TSS_SUCCESS;
  ...
  *random_data = ConvertByteBufferToString(random_bytes, num_bytes);
  Tspi_Context_FreeMemory(tsp_context_, random_bytes);
  VLOG(1) << "TPMUtilityNetHsm::GenerateRandom success";
  return true;
}

bool TPMUtilityNetHsm::StirRandom(const string& entropy_data) {
  VLOG(1) << "TPMUtilityNetHsm::StirRandom enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  TSS_RESULT result = TSS_SUCCESS;
  ...
  VLOG(1) << "TPMUtilityNetHsm::StirRandom success";
  return true;
}

bool TPMUtilityNetHsm::GenerateKey(int slot,
                                 int modulus_bits,
                                 const string& public_exponent,
                                 const SecureBlob& auth_data,
                                 string* key_blob,
                                 int* key_handle) {
  VLOG(1) << "TPMUtilityNetHsm::GenerateKey enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  TSS_RESULT result = TSS_SUCCESS;
  ...
  *key_handle = CreateHandle(slot, key.release(), *key_blob, auth_data);
  VLOG(1) << "TPMUtilityNetHsm::GenerateKey success";
  return true;
}

bool TPMUtilityNetHsm::GetPublicKey(int key_handle,
                                  string* public_exponent,
                                  string* modulus) {
  VLOG(1) << "TPMUtilityNetHsm::GetPublicKey enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  ...
  if (!GetKeyAttributeData(GetTssHandle(key_handle),
                           ...
                           modulus))
    return false;
  VLOG(1) << "TPMUtilityNetHsm::GetPublicKey success";
  return true;
}

bool TPMUtilityNetHsm::WrapKey(int slot,
                             const string& public_exponent,
                             ...
                             string* key_blob,
                             int* key_handle) {
  VLOG(1) << "TPMUtilityNetHsm::WrapKey enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  if (!GetSRKPublicKey())
  ...
  if (!GetKeyBlob(key, key_blob))
    return false;
  *key_handle = CreateHandle(slot, key.release(), *key_blob, auth_data);
  VLOG(1) << "TPMUtilityNetHsm::WrapKey success";
  return true;
}

bool TPMUtilityNetHsm::LoadKey(int slot,
...
bool TPMUtilityNetHsm::LoadKeyWithParent(int slot,
                                       const string& key_blob,
                                       const SecureBlob& auth_data,
  ...
  if (IsAlreadyLoaded(slot, key_blob, key_handle))
    return true;
  VLOG(1) << "TPMUtilityNetHsm::LoadKeyWithParent enter";
  ScopedTssKey key(tsp_context_);
  if (!LoadKeyInternal(GetTssHandle(parent_key_handle), key_blob, auth_data,
                       key.ptr()))
    return false;
  *key_handle = CreateHandle(slot, key.release(), key_blob, auth_data);
  VLOG(1) << "TPMUtilityNetHsm::LoadKeyWithParent success";
  return true;
}

void TPMUtilityNetHsm::UnloadKeysForSlot(int slot) {
  VLOG(1) << "TPMUtilityNetHsm::UnloadKeysForSlot enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return;
  ...
  VLOG(1) << "TPMUtilityNetHsm::UnloadKeysForSlot success";
}

bool TPMUtilityNetHsm::Bind(int key_handle,
                          const string& input,
                          string* output) {
  VLOG(1) << "TPMUtilityNetHsm::Bind enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  ...
  if (!encrypted.GetData(output))
    return false;
  VLOG(1) << "TPMUtilityNetHsm::Bind success";
  return true;
}

bool TPMUtilityNetHsm::Unbind(int key_handle,
                            const string& input,
                            string* output) {
  VLOG(1) << "TPMUtilityNetHsm::Unbind enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  TSSEncryptedData encrypted(tsp_context_);
  ...
  *output = ConvertByteBufferToString(buffer, length);
  ...
  VLOG(1) << "TPMUtilityNetHsm::Unbind success";
  return true;
}

bool TPMUtilityNetHsm::Sign(int key_handle,
                          const string& input,
                          string* signature) {
  VLOG(1) << "TPMUtilityNetHsm::Sign enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  ...
  *signature = ConvertByteBufferToString(buffer, length);
  Tspi_Context_FreeMemory(tsp_context_, buffer);
  VLOG(1) << "TPMUtilityNetHsm::Sign success";
  return true;
}

bool TPMUtilityNetHsm::Verify(int key_handle,
                            const string& input,
                            const string& signature) {
  VLOG(1) << "TPMUtilityNetHsm::Verify enter";
  AutoLock lock(lock_);
  if (!InitSRK())
    return false;
  TSSHash hash(tsp_context_);
  ...
  VLOG(1) << "TPMUtilityNetHsm::Verify success";
  return true;
}

bool TPMUtilityNetHsm::IsSRKReady() {
  VLOG(1) << "TPMUtilityNetHsm::IsSRKReady";
  AutoLock lock(lock_);
  return InitSRK();
}

int TPMUtilityNetHsm::CreateHandle(int slot,
                                 TSS_HKEY key,
                                 const string& key_blob,
                                 const SecureBlob& auth_data) {
...
bool TPMUtilityNetHsm::CreateKeyPolicy(TSS_HKEY key,
                                     const SecureBlob& auth_data,
                                     bool auth_only) {
  ScopedTssPolicy policy(tsp_context_);
...
bool TPMUtilityNetHsm::GetKeyAttributeData(TSS_HKEY key,
                                         TSS_FLAG flag,
                                         TSS_FLAG sub_flag,
                                         string* data) {
  UINT32 length = 0;
...
bool TPMUtilityNetHsm::GetKeyBlob(TSS_HKEY key, string* blob) {
...
TSS_FLAG TPMUtilityNetHsm::GetKeyFlags(int modulus_bits) {
...
bool TPMUtilityNetHsm::GetSRKPublicKey() {
...
TSS_HKEY TPMUtilityNetHsm::GetTssHandle(int key_handle) {
...
bool TPMUtilityNetHsm::IsAlreadyLoaded(int slot,
                                     const string& key_blob,
                                     int* key_handle) {
  HandleInfo* handle_info = &slot_handles_[slot];
...
bool TPMUtilityNetHsm::LoadKeyInternal(TSS_HKEY parent,
                                     const string& key_blob,
                                     const SecureBlob& auth_data,
                                     TSS_HKEY* key) {
...
bool TPMUtilityNetHsm::ReloadKey(int key_handle) {
...
string TPMUtilityNetHsm::ResultToString(TSS_RESULT result) {
  if (result == TSS_SUCCESS)
    return "TSS_SUCCESS";
